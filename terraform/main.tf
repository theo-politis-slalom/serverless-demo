...
# S3 access policy for Lambda code
# S3 bucket for Lambda deployment packages
resource "aws_s3_bucket" "lambda_bucket" {
  bucket = "${var.project_name}-lambda-${var.environment}-${data.aws_caller_identity.current.account_id}"
  
  tags = merge(
    local.common_tags,
    {
      Name = "${var.project_name}-lambda-bucket-${var.environment}"
    }
  )
}
...
resource "aws_iam_policy" "lambda_s3_access" {
  ...
  description = "IAM policy for Lambda to access S3 deployment bucket"
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:s3:::${var.project_name}-lambda-${var.environment}-*",
          "arn:aws:s3:::${var.project_name}-lambda-${var.environment}-*/*"
        ]
      }
    ]
...
resource "aws_iam_role_policy_attachment" "name_lambda_s3" {
  role       = aws_iam_role.name_lambda_exec.name
...
resource "aws_iam_role_policy_attachment" "health_lambda_s3" {
  role       = aws_iam_role.health_lambda_exec.name
  policy_arn = aws_iam_policy.lambda_s3_access.arn
}
...
resource "aws_iam_role_policy_attachment" "secrets_demo_lambda_s3" {
  role       = aws_iam_role.secrets_demo_lambda_exec.name
  policy_arn = aws_iam_policy.lambda_s3_access.arn
}
...
resource "aws_lambda_function" "name" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.name_lambda_s3,
...
resource "aws_lambda_function" "health_check" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.health_lambda_s3,
    aws_cloudwatch_log_group.lambda_logs,
    null_resource.build_lambda,
  ]
}
...
resource "aws_lambda_function" "secrets_manager_demo" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.secrets_demo_lambda_s3,
